...
function renderTables(){const box=document.querySelector("#table-buttons");if(!box)return;box.innerHTML="";for(let n=2;n<=10;n++){const b=document.createElement("button");b.className="table-btn "+(selected.has(n)?"selected":"");b.textContent=n+"er";b.onclick=()=>{selected.has(n)?selected.delete(n):selected.add(n);renderTables()};box.appendChild(b)}}
